...
const oldLoadEmails = `window.loadEmails = async () => {
...
window.loadEmails = async (folder = 'inbox') => {
  ...
  try {
    ...
    const emails = result.emails || [];
    window.currentEmailsList = emails; // store for detail view
    ...
    emails.forEach((email, idx) => {
      ...
      // Highlight unread emails (only relevant for inbox/unread folders)
...
if(code.includes(oldLoadEmails)) {
  code = code.replace(oldLoadEmails, newLoadEmails);
  fs.writeFileSync('js/admin.js', code);
  console.log("admin.js loadEmails patched");
} else {
  console.log("Could not find oldLoadEmails");
}
