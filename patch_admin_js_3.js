...
const endIndex = code.indexOf('};', code.indexOf('emailsList.innerHTML = "<p style=\'color:red;\'>Помилка: " + error.message + "</p>";', startIndex)) + 2;
...
window.loadEmails = async (folder = 'inbox') => {
  ...
  // Now I must also fix `window.showEmailDetail` to read from `window.currentEmailsList` instead of fetching it again.
  // Wait, does it fetch again?
