...
// The file currently has a corrupted GET /api/emails followed by the remainder of the IMAP code.
// We can just find the start of GET /api/emails and the start of POST /api/send-reply-email.
const startGet = code.indexOf("app.get('/api/emails'");
const startSend = code.indexOf("app.post('/api/send-reply-email'");

if (startGet > -1 && startSend > -1) {
  const beforeGet = code.substring(0, startGet);
  // Also we want to replace the POST /api/send-reply-email up to OAuth2 Email
  const startOAuth = code.indexOf("// ===== OAuth2 Email (Gmail) =====");
  
  const afterSend = startOAuth > -1 ? code.substring(startOAuth) : "";

  const newCode = beforeGet + `app.get('/api/emails', async (req, res) => {
  try {
...
app.post('/api/send-reply-email', async (req, res) => {
  ...
  console.log("Could not find startGet or startSend");
}
