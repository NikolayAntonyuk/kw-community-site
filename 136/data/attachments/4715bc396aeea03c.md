...
- Name: catalog-improvements.spec.ts >> Catalog Improvements >> 1. Phone display: Shows only number without 'Телефон:' label
- Location: tests/e2e/catalog-improvements.spec.ts:9:7
...
Call log:
  - waiting for locator('.cards-grid button').first()
...
```ts
  ...
  5   |     await page.goto("/catalog.html");
